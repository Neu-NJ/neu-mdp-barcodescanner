...
@property (nonatomic, strong) UIImageView *scanCropView;
...
- (void)viewWillDisappear:(BOOL)animated {
    [[UIApplication sharedApplication] setStatusBarStyle:self.originStatusBarStyle animated:NO];
...
- (void)loadCustomView {
    ...
    UIView* nav = [[UIView alloc] initWithFrame:CGRectMake(0, 0, rc.size.width, 64)];

    nav.backgroundColor = [UIColor colorWithRed:(94/255.0) green:(127/255.0) blue:(255/255.0) alpha:1];
    ...
    //中间扫描区域
    self.scanCropView=[[UIImageView alloc] initWithFrame:CGRectMake(_width, _height, rc.size.width - _width - _width, rc.size.height - _height - _height)];
   self.scanCropView.image=[UIImage imageNamed:@"kuang.jpg"];
    self.scanCropView. backgroundColor =[ UIColor clearColor ];
...
- (void)moveUpAndDownLine {
    ...
    CGFloat x = CGRectGetMinX(self.scanCropView.frame);
    CGFloat y = CGRectGetMinY(self.scanCropView.frame);
    CGFloat w = CGRectGetWidth(self.scanCropView.frame);
    CGFloat h = CGRectGetHeight(self.scanCropView.frame);
    //用于线条移动
